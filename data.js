const ABASE = [
 ...
 ["e","[ɛ], близко к русскому э; без редукции в безударной позиции"],["f","как русское ф"],["g","обычно [g], не английское [дж] перед e/i: gest, gitara; перед i смягчается: drogi"],
 ["h","как русское х, см. пару ch/h ниже"],["i","как русское и; после согласной обычно показывает её мягкость"],
 ...
 ["m","как русское м"],["n","как русское н"],["o","[ɔ], без перехода в а в безударной позиции"],["p","как русское п"],
 ["r","раскатистое р, как в русском"],["s","как русское с"],["t","как русское т"],
 ["u","как русское у, см. пару u/ó ниже"],["w","в: woda - «вода»"],
 ["y","ы; в обычной польской лексике стоит после твёрдой согласной и не смягчает её"],["z","как русское з"]
];
const ADIAC = [
 ["ą","см. ниже","буква для заднего носового ряда","не имеет одного чтения: реализация зависит от следующего звука и позиции в слове","dąb, mąka, idą"],
 ["ę","см. ниже","буква для переднего носового ряда","не имеет одного чтения: реализация зависит от следующего звука и позиции в слове","zęby, ręka, proszę"],
 ["ć","[t͡ɕ]","мягкое ч","короткий мягкий аффрикат, близкий к русскому ч; перед гласной записывается через ci","być, ćma"],
 ["ł","[w]","неслоговое у","губной звук, как белорусское ў или английское w; это не л","łyżka, mały"],
 ["ń","[ɲ]","мягкое нь","близко к нь в слове «конь»; перед гласной записывается через ni","dzień, koń"],
 ["ó","[u]","у","произносится так же, как u; различие только на письме","król, wróbel"],
 ["ś","[ɕ]","короткое мягкое щ","близко к русскому щ, но без его долготы; перед гласной записывается через si","śniadanie, gość"],
 ["ź","[ʑ]","звонкая пара к ś","в русском нет точного отдельного соответствия; перед гласной записывается через zi","źle, wiozę"],
 ["ż","[ʐ]","твёрдое ж","близко к русскому ж; обычно совпадает по звуку с rz","żaba, może"]
];
const DIGR = [
 ["sz","[ʂ]","твёрдое ш","szkoła, kasza"],
 ["cz","[t͡ʂ]","твёрдое ч","czas, klucz"],
 ["rz","[ʐ]","ж (= ż)","rzeka, morze"],
 ["dz","[d͡z]","дз слитно","dzwon, władza"],
 ["dź","[d͡ʑ]","мягкое дж","dźwięk, łódź"],
 ["dż","[d͡ʐ]","твёрдое дж","dżem, dżungla"],
 ["ch","[x]","х (= h)","chleb, dach"]
];
const LETTER_GROUPS = [
 ["dzi","триграф","[d͡ʑ]","тот же звук, что dź, перед гласной","dziecko, nadzieja"],
 ["szcz","два диграфа","[ʂt͡ʂ]","два твёрдых звука: sz + cz","szczęście, deszcz, jeszcze"]
];
const PALAT = [
 ["ś / si","śniadanie - gość","siostra - siano","silny - siwy"],
 ["ć / ci","być - ćma","ciocia - ciasto","cichy - cisza"],
 ["ź / zi","źle - paź","ziemia - ziarno","zima - zimny"],
 ["ń / ni","koń - słońce","niebo - konie","nitka - niski"],
 ["dź / dzi","łódź - dźwig","dziecko - godzina","dziwny - podziw"]
];
const NASAL = [
 ["перед b, p","[om], [em]","dąb [domp], zęby [zemby]"],
 ["перед d, t, dz, c","[on], [en]","kąt [kont], ręce [rence]"],
 ["перед cz, dż","[on], [en], но n подстраивается под следующий звук","pączek: носовая согласная произносится в месте образования cz"],
 ["перед ć/ci, dź/dzi","[oɲ], [eɲ]","będzie: [eɲ] перед dź"],
 ["перед g, k","[oŋ], [eŋ]","mąka [moŋka], ręka [reŋka]"],
 ["перед f, w, s, z, ś, ź, sz, rz, ż, ch/h","носовой переход без отдельного n или m","wąs [võũ̯s], węch [vẽũ̯x]"],
 ["перед ł, l","носовость теряется: [o], [e]","minął [minow], zaczęli [zat͡ʂeli]"],
 ["на конце слова","ą сохраняет носовой переход; у ę носовость ослабляется или исчезает","idą [idõũ̯]; proszę с конечным [ɛ] или слабой носовостью"]
];
...
const DZW_RULES = [
 ...
 ["перед глухим препятственным","соседний звонкий обычно оглушается","ławka [łafka] · babka [bapka] · łódka [łutka] · wszystko [fszystko] · wtorek [ftorek]"],
 ["перед звонким препятственным","соседний глухой обычно озвончается","także [tagże] · prośba [proźba] · liczba [lidżba] · jakże [jagże]"],
 ["на границе предлога и слова","ассимиляция может переходить границу слов","w parku [f parku] · z tobą [s tobą] · nad stołem [nat stołem] · pod domem [pod domem]"]
];
const WIELKA_D = [
...
const STRESS_EXC = [
 ...
 ["греко-латинские слова на -yka / -ika в образцовой речи","3-й слог от конца","fizyka, matematyka, gramatyka; в разговорной речи встречается предпоследний"],
...
const ORTHO_CH = [
 ["ch","чередуется с sz","słuch → słyszeć · suchy → susza · pociecha → pocieszyć"],
 ["ch","после s","schody, schab, schowek"],
 ["ch","в окончании Miejscownik множественного -ach","w miastach, o stołach, na ulicach"],
 ["ch","на конце исконных и давно освоенных слов","dach, grzech, krach; исключение: druh"],
 ["h","чередуется с g, z, ż","waga → waha się · błazen → błahy · druh → drużyna"],
 ["ch / h","во многих заимствованиях написание словарное","chemia, charakter, mechanik · higiena, hotel, harfa, historia"]
];
const ORTHO_RZ = [
 ["rz","чередуется с r","góra → górzysty · wzorzec → wzorcowy · morze → morski"],
 ["rz","после b, p, d, t, g, k, ch, w, j","przyjaciel, trzy, krzak, wrzawa, grzyb"],
 ["не rz","частые исключения после этих согласных","pszczoła, pszenica, wszystko, kształt, bukszpan"],
 ["rz","в суффиксах -arz, -erz, -mierz","malarz, tancerz, żołnierz"],
 ["ż","чередуется с g, z, ź/zi, dz, dź, s","odwaga → odważny · pieniądz → spieniężyć · groźny → grożę · zwycięstwo → zwyciężać"],
...
const CASES = [
{
  ...
  sg:[
    ...
    {l:"муж. на -a", f:[{a:"",b:"tat|a"},{a:"",b:"koleg|a"},{a:"",b:"mężczyzn|a"},{a:"",b:"kierowc|a"}], n:"в ед. ч. склоняются по-женски, во мн. ч. - по мужско-личному; согласование всегда мужское: ten kolega, ci koledzy"}
  ],
  pl:[
    {l:"мужско-личный", f:[{a:"student",b:"studen|c|i"},{a:"sąsiad",b:"sąsie|dzi|"},{a:"Polak",b:"Pola|c|y"},{a:"lekarz",b:"lekarz|e"},{a:"pan",b:"pan|owie"},{a:"tata",b:"tat|owie"},{a:"kolega",b:"kole|dz|y"},{a:"mężczyzna",b:"mężczy|źn|i"}], n:"мужские названия лиц и смешанные группы людей: -i / -y / -e / -owie, часто с чередованием"},
  ...
  agree:[["ten dobry sklep","ten dobry sklep"],["ten dobry tata","ten dobry tata"],["ta dobra kawa","ta dobra kawa"],["to dobre piwo","to dobre piwo"],["мн. мужчины","ci dobrzy studenci"],["мн. остальное","te dobre książki"]],
...
},
{
  ...
  sg:[
    ...
    {l:"муж. на -a", f:[{a:"tata",b:"tat|ę"},{a:"kolega",b:"koleg|ę"},{a:"mężczyzna",b:"mężczyzn|ę"},{a:"kierowca",b:"kierowc|ę"}], n:"-ę, как у kawa: widzę tatę, не taty. Прилагательное при этом мужское: mojego tatę"},
  ...
  pl:[
    {l:"мужско-личный", f:[{a:"panowie",b:"pan|ów"},{a:"lekarze",b:"lekarz|y"},{a:"nauczyciele",b:"nauczyciel|i"},{a:"studenci",b:"student|ów"},{a:"sąsiedzi",b:"sąsiad|ów"},{a:"przyjaciele",b:"przyjaciół|"},{a:"tatowie",b:"tat|ów"},{a:"koledzy",b:"koleg|ów"},{a:"mężczyźni",b:"mężczyzn|"}], n:"= родительный"},
  ...
  agree:[["ten dobry chleb","ten dobry chleb"],["ten dobry pies","tego dobrego psa"],["ten dobry tata","tego dobrego tatę"],["ta dobra kawa","tę dobrą kawę"],["to dobre piwo","to dobre piwo"],["мн. мужчины","tych dobrych lekarzy"],["мн. остальное","te dobre kawy"]],
  ...
  pit:["<b>Как выбрать форму.</b> В единственном числе проверь род: мужское неодушевлённое и среднее совпадают с именительным (<span class='pl'>widzę dom, auto</span>), мужское одушевлённое - с родительным (<span class='pl'>widzę psa</span>), кроме мужских на <span class='pl'>-a</span>: у них <span class='pl'>-ę</span>, как у женских, а прилагательное мужское (<span class='pl'>widzę mojego tatę, kolegę</span>), женское на <span class='pl'>-a</span> получает <span class='pl'>-ę</span> (<span class='pl'>widzę kawę</span>). Во множественном только группа мужчин совпадает с родительным (<span class='pl'>widzę studentów</span>); всё остальное, включая животных, - с именительным (<span class='pl'>widzę psy, książki</span>).",
...
},
{
  ...
  sg:[
    ...
    {l:"муж. на -a", f:[{a:"tata",b:"tat|y"},{a:"kolega",b:"koleg|i"},{a:"mężczyzna",b:"mężczyzn|y"},{a:"kierowca",b:"kierowc|y"}], n:"как женские: -y, после k, g -i. Прилагательное мужское: tego dobrego taty"},
  ...
  pl:[
    ...
    {l:"муж. на -a", f:[{a:"tatowie",b:"tat|ów"},{a:"koledzy",b:"koleg|ów"},{a:"kierowcy",b:"kierowc|ów"},{a:"mężczyźni",b:"mężczyzn|"}], n:"как мужские: -ów; mężczyzna - без окончания: mężczyzn"},
  ...
  agree:[["ten dobry chleb","tego dobrego chleba"],["ten dobry tata","tego dobrego taty"],["ta dobra kawa","tej dobrej kawy"],["to dobre piwo","tego dobrego piwa"],["мн. любое","tych dobrych ludzi / tych dobrych kaw"]],
...
},
{
  ...
  sg:[
    ...
    {l:"муж. на -a", f:[{a:"tata",b:"ta|ci|e"},{a:"kolega",b:"kole|dz|e"},{a:"mężczyzna",b:"mężczy|źni|e"},{a:"kierowca",b:"kierowc|y"}], n:"как женские, а не -owi: tacie, koledze. Прилагательное мужское: temu dobremu tacie"},
  ...
  agree:[["ten dobry student","temu dobremu studentowi"],["ten dobry tata","temu dobremu tacie"],["ta dobra kobieta","tej dobrej kobiecie"],["to dobre dziecko","temu dobremu dziecku"],["мн. любое","tym dobrym ludziom"]],
...
},
{
  ...
  sg:[
    ...
    {l:"муж. на -a", f:[{a:"tata",b:"tat|ą"},{a:"kolega",b:"koleg|ą"},{a:"mężczyzna",b:"mężczyzn|ą"},{a:"kierowca",b:"kierowc|ą"}], n:"-ą, как у женских, а не -em. Прилагательное мужское: z tym dobrym kolegą"}
  ],
  pl:[
  ...
  agree:[["ten dobry lekarz","tym dobrym lekarzem"],["ten dobry kolega","tym dobrym kolegą"],["ta dobra kawa","tą dobrą kawą"],["to dobre piwo","tym dobrym piwem"],["мн. любое","tymi dobrymi ludźmi"]],
...
},
{
  ...
  sg:[
    ...
    {l:"муж. на -a", f:[{a:"tata",b:"ta|ci|e"},{a:"kolega",b:"kole|dz|e"},{a:"mężczyzna",b:"mężczy|źni|e"},{a:"kierowca",b:"kierowc|y"}], n:"как женские, а не -u: o tacie, o koledze. Прилагательное мужское: o moim tacie"}
  ],
  ...
  agree:[["ten duży sklep","w tym dużym sklepie"],["mój dobry tata","o moim dobrym tacie"],["ta nowa praca","w tej nowej pracy"],["to małe miasto","w tym małym mieście"],["мн. любое","w tych dużych sklepach"]],
...
},
{
  ...
  sg:[
    ...
    {l:"муж. на -a", f:[{a:"tata",b:"tat|o"},{a:"kolega",b:"koleg|o"},{a:"kierowca",b:"kierowc|o"}], n:"-o, как у женских: Tato! Kolego! Прилагательное мужское: kochany tato"},
  ...
  agree:[["drogi Marek","drogi Marku"],["kochany tata","kochany tato"],["droga Anna","droga Anno"],["szanowni panowie","szanowni panowie"]],
...
const KON = [
 ...
 {n:"IV", mark:"-em / -esz",
  who:"Закрытый список глаголов и их приставочных производных. Учится списком.",
  ...
  verbs:[
   ...
   ["śmieć","сметь, осмеливаться",["śmiem","śmiesz","śmie","śmiemy","śmiecie","śmieją"],"Jak śmiesz! - не путать со śmiać się: śmieję się"],
...
const REKCJA = [
 ...
 ["ożenić się","z kim?","z + Narzędnik","жениться на ком","Ożenił się z Anną.",1],
...
const POTOCZNE = [
 ["w sumie","в общем, если подумать","W sumie masz rację. · W sumie to nie wiem.","Подводит итог или смягчает мнение. Разговорное, но не грубое."],
 ["po prostu","просто","Po prostu nie chcę. · To po prostu działa.","Единственное в списке, что спокойно идёт и в письменную речь."],
 ["właściwie","собственно, вообще-то","Właściwie to nie mój problem. · O co właściwie chodzi?","Вводит уточнение или мягкое возражение."],
 ["tak naprawdę","на самом деле","Tak naprawdę chodzi o pieniądze.","Очень частотно в речи. В тексте почти всегда убирается без потери смысла."],
 ["w każdym razie","в любом случае","W każdym razie zadzwoń wieczorem.","Возвращает к главному после отступления."],
 ["swoją drogą","кстати, между прочим","Swoją drogą, to dobry pomysł.","Вводит побочное замечание, часто с одобрением."],
 ["przy okazji","заодно, кстати","Przy okazji, masz chwilę?","Прицепляет просьбу или вопрос к уже идущему разговору."],
 ["chodzi o to, że","дело в том, что","Chodzi o to, że nie mam czasu.","Рабочий зачин объяснения или отказа."],
 ["że tak powiem","так сказать","To był, że tak powiem, eksperyment.","Оговорка при неточном слове. Выделяется запятыми."],
 ["powiem szczerze","скажу честно","Powiem szczerze, nie podoba mi się.","Готовит собеседника к неприятной правде."],
 ["wiesz co","знаешь что","Wiesz co, chodźmy stąd.","Вводит предложение или смену темы. Само <span class='pl'>wiesz</span> без <span class='pl'>co</span> легко превращается в паразита."],
 ["no nie wiem","ну не знаю","No nie wiem, zobaczymy.","Вежливое сомнение вместо прямого «нет»."],
 ["no dobra","ну ладно","No dobra, niech będzie.","Согласие с уступкой. Разговорное."],
 ["jakoś","как-то, как-нибудь","Jakoś nie mam ochoty. · Jakoś to będzie.","Смягчает отказ. <span class='pl'>Jakoś to będzie</span> - «как-нибудь обойдётся»."],
 ["nie ma sprawy","без проблем","- Dzięki za pomoc. - Nie ma sprawy.","Ответ на благодарность или необременительную просьбу."],
 ["spoko","спокойно, окей","Spoko, poczekam.","Молодёжно-разговорное. С незнакомым человеком и в официальной обстановке неуместно."],
 ["tak że","так что","Nie zdążyłem, tak że wrócę jutro.","Раздельно и с запятой перед ним. Слитное <span class='pl'>także</span> - это «также», другое слово."]
];
const REPLIKI = [
 ["reply","вот именно",["No właśnie","Właśnie","Otóż to"],"собеседник сказал ровно то, что вы имели в виду","Нейтральное подтверждение. Кальку Dokładnie! тоже услышите, но языковеды советуют вместо неё No właśnie."],
 ["reply","ну ладно",["No dobra","No dobrze"],"соглашаетесь с лёгкой уступкой","No dobra разговорнее, No dobrze чуть сдержаннее. Без no это уже просто «хорошо»."],
 ["reply","без проблем",["Nie ma sprawy","Nie ma problemu"],"отвечаете на благодарность или необременительную просьбу, нейтрально","Обе формы равноправны. Молодёжное Spoko звучит свободнее и с незнакомым человеком не годится."],
 ["reply","ну ясно",["No jasne"],"показываете, что поняли и согласны","Готовая реплика из ряда No dobra, No pewnie, No jasne."],
 ["reply","ну разумеется",["Ależ oczywiście"],"подтверждаете с нажимом: странно, что вопрос вообще возник","Ależ усиливает и добавляет эмоцию, само oczywiście нейтральнее."],
 ["reply","я же говорил",["Przecież mówiłem","Przecież mówiłam"],"напоминаете о своём предупреждении; выберите форму по своему роду","Przecież вводит напоминание о известном. Женская форма: Przecież mówiłam."],
 ["reply","сойдёт, пойдёт",["Może być"],"принимаете предложенное без восторга","Устойчивый оборот. Дословно «может быть», но значит «годится»."],
 ["reply","ага, как же",["Akurat"],"иронично не верите сказанному","Akurat с иронической интонацией означает недоверие. В нейтральном значении это «как раз, ровно столько»."],
 ["reply","серьёзно?",["Serio","Naprawdę","Poważnie"],"удивляетесь услышанному","Три равноправных варианта. Serio разговорнее всего."],
 ["link","в общем, если подумать",["W sumie"],"подводите итог сказанному или смягчаете своё мнение","W sumie masz rację. Дословно «в сумме», отсюда значение «если сложить всё вместе»."],
 ["link","на самом деле",["Tak naprawdę"],"вводите то, что считаете настоящей причиной","Очень частотно в речи. В письменном тексте почти всегда убирается без потери смысла."],
 ["link","дело в том, что",["Chodzi o to, że","Chodzi o to że"],"начинаете объяснение или отказ","Дословно «речь идёт о том, что». Перед że в польском ставится запятая."],
 ["link","кстати, между прочим",["Swoją drogą","Nawiasem mówiąc"],"вводите побочное замечание, часто с одобрением","Swoją drogą разговорнее, nawiasem mówiąc книжнее."],
 ["link","заодно, раз уж зашла речь",["Przy okazji"],"прицепляете просьбу или вопрос к идущему разговору","Przy okazji, masz chwilę? Отличается от swoją drogą тем, что вводит не замечание, а дело."],
 ["link","в любом случае",["W każdym razie","Tak czy inaczej","Tak czy owak"],"возвращаетесь к главному после отступления","Три равноправных оборота. W każdym razie самый частый."],
 ["link","так что",["Tak że"],"вводите вывод из сказанного","Пишется раздельно и с запятой перед ним: Nie zdążyłem, tak że wrócę jutro. Слитное także значит «также»."],
 ["link","знаешь что",["Wiesz co"],"меняете тему или вносите предложение","Wiesz co, chodźmy stąd. Одно wiesz без co легко превращается в слово-паразит."],
 ["link","то есть",["To znaczy","Czyli"],"поправляете себя или поясняете сказанное","Разговорное znaczy się тоже слышно, но на письме употребляют to znaczy."],
 ["soften","ну не знаю",["No nie wiem"],"вежливо сомневаетесь вместо прямого «нет»","Мягче, чем Nie. Оставляет собеседнику место для возражения."],
 ["soften","скорее нет",["Raczej nie","Chyba nie"],"мягко отказываете","Raczej nie звучит чуть решительнее, chyba nie оставляет больше сомнения."],
 ["soften","как-то",["Jakoś"],"смягчаете отказ: как-то не хочется","Jakoś nie mam ochoty. Снимает категоричность, не называя причины."],
 ["soften","просто",["Po prostu"],"подчёркиваете, что причина простая и объяснять нечего","Po prostu nie chcę. Единственный оборот из этого ряда, уместный и в письменной речи."],
 ["soften","вообще-то, собственно",["Właściwie"],"вводите уточнение или мягкое возражение","Właściwie to nie mój problem. Смягчает несогласие."],
 ["soften","скажу честно",["Powiem szczerze"],"готовите собеседника к неприятной правде","Powiem szczerze, nie podoba mi się. Предупреждает, что дальше прямая оценка."],
 ["soften","так сказать",["Że tak powiem"],"оговариваетесь, что слово неточное","To był, że tak powiem, eksperyment. Выделяется запятыми с двух сторон."],
 ["soften","как-нибудь обойдётся",["Jakoś to będzie"],"успокаиваете: всё уладится само","Устойчивая фраза, почти поговорка."],
 ["soften","дай бог, хоть бы",["Oby"],"выражаете надежду одним словом","Oby! как отдельная реплика или Oby się udało перед сказуемым."]
];
const REKCJA_TRAIN = [
 ["verb","Dziękuję ___ za pomoc. (ty)",["ci","tobie"],"благодарить кого","Dziękować требует Celownik, а не Biernik: dziękuję ci. Полное tobie - при выделении.",1],
 ["verb","Pomagam ___. (mama)",["mamie"],"помогать кому","Pomagać + Celownik, как и в русском: pomagam mamie."],
 ["verb","Dzwonię ___. (brat)",["do brata"],"звонить кому","Русское «звонить кому» тянет к Celownik, но польский требует do + Dopełniacz.",1],
 ["verb","Szukam ___. (praca)",["pracy"],"искать что","Szukać + Dopełniacz. Русский Biernik здесь не работает: не szukam pracę.",1],
 ["verb","Słucham ___. (muzyka)",["muzyki"],"слушать что","Słuchać + Dopełniacz: słucham muzyki, а не muzykę.",1],
 ["verb","Uczę się ___. (polski)",["polskiego"],"учить что","Uczyć się + Dopełniacz: uczę się polskiego.",1],
 ["verb","Używam ___. (telefon)",["telefonu"],"пользоваться чем","Używać + Dopełniacz. Русский Narzędnik сюда не переносится.",1],
 ["verb","Potrzebuję ___. (pomoc)",["pomocy"],"нуждаться в чём","Potrzebować + Dopełniacz: potrzebuję pomocy.",1],
 ["verb","Zapomniałem ___. (klucze)",["kluczy","o kluczach"],"забыть что","Zapomnieć + Dopełniacz. С предлогом другая модель: zapomniałem o spotkaniu.",1],
 ["verb","Boję się ___. (psy)",["psów"],"бояться чего","Bać się + Dopełniacz, как и в русском."],
 ["verb","Cieszę się ___. (prezent)",["z prezentu"],"радоваться чему","Cieszyć się z + Dopełniacz. Русский Celownik здесь не подходит.",1],
 ["verb","Korzystam ___. (internet)",["z internetu"],"пользоваться чем","Korzystać z + Dopełniacz. Синоним używać идёт без предлога.",1],
 ["verb","Śmieję się ___. (to)",["z tego"],"смеяться над чем","Śmiać się z + Dopełniacz, а не «над». Так же żartować z kogoś.",1],
 ["verb","Jestem ___ dumny. (ty)",["z ciebie"],"гордиться кем","Być dumnym z + Dopełniacz.",1],
 ["verb","Egzamin składa się ___. (dwie części)",["z dwóch części"],"состоять из чего","Składać się z + Dopełniacz. После dwie в Dopełniacz: dwóch części."],
 ["verb","Choruję ___. (grypa)",["na grypę"],"болеть чем","Chorować na + Biernik: choruję na grypę.",1],
 ["verb","Czekam ___. (autobus)",["na autobus"],"ждать кого или что","Czekać na + Biernik. Русское «ждать автобуса» без предлога не переносится.",1],
 ["verb","Patrzę ___. (zdjęcie)",["na zdjęcie"],"смотреть на что","Patrzeć na + Biernik, как и в русском."],
 ["verb","Zapraszam cię ___. (kawa)",["na kawę"],"приглашать на что","Zapraszać na + Biernik: zapraszam na kawę."],
 ["verb","Proszę ___. (pomoc)",["o pomoc"],"просить о чём","Prosić o + Biernik. Русское «просить помощи» без предлога сюда не переносится.",1],
 ["verb","Pytam ___. (cena)",["o cenę"],"спрашивать о чём","Pytać o + Biernik: pytam o cenę.",1],
 ["verb","Martwię się ___. (ty)",["o ciebie"],"беспокоиться о ком","Martwić się o + Biernik, а не o + Miejscownik.",1],
 ["verb","Dbam ___. (zdrowie)",["o zdrowie"],"заботиться о чём","Dbać o + Biernik: dbam o zdrowie.",1],
 ["verb","Interesuję się ___. (muzyka)",["muzyką"],"интересоваться чем","Interesować się + Narzędnik, как и в русском."],
 ["verb","Zajmuję się ___. (dzieci)",["dziećmi"],"заниматься чем","Zajmować się + Narzędnik. Множественное от dziecko: dziećmi."],
 ["verb","Opiekuję się ___. (babcia)",["babcią"],"ухаживать за кем","Opiekować się + Narzędnik без предлога: opiekuję się babcią.",1],
 ["verb","Rozmawiam ___ o pracy. (szef)",["z szefem"],"разговаривать с кем","Rozmawiać z + Narzędnik, тема через o + Miejscownik."],
 ["verb","Spotykam się ___. (przyjaciele)",["z przyjaciółmi"],"встречаться с кем","Spotykać się z + Narzędnik. Множественное: z przyjaciółmi."],
 ["verb","Tęsknię ___. (dom)",["za domem"],"скучать по чему","Tęsknić za + Narzędnik, а не «по».",1],
 ["verb","Zakochałem się ___. (ona)",["w niej"],"влюбиться в кого","Zakochać się w + Miejscownik. Русский Biernik сюда не переносится.",1],
 ["verb","Jestem podobny ___. (ojciec)",["do ojca"],"быть похожим на кого","Być podobnym do + Dopełniacz, а не «на».",1],
 ["verb","Wierzę ___. (ty)",["w ciebie","ci"],"верить во что и верить кому","Две разные модели: wierzę w ciebie - верю в тебя, wierzę ci - верю тебе."],
 ["verb","Gratuluję ci ___. (sukces)",["sukcesu"],"поздравлять с чем","Gratulować + Celownik лица и Dopełniacz повода: gratuluję ci sukcesu.",1],
 ["verb","Życzę ci ___. (zdrowie)",["zdrowia"],"желать чего","Życzyć + Celownik лица и Dopełniacz предмета."],
 ["verb","Gram ___. (piłka)",["w piłkę"],"играть во что","Grać w + Biernik об игре, grać na + Miejscownik об инструменте.",1],
 ["verb","Idę ___. (chleb)",["po chleb"],"идти за чем","Iść po + Biernik означает «сходить за».",1],
 ["adjective","Jestem dumny ___. (syn)",["z syna"],"гордиться кем","Dumny z + Dopełniacz."],
 ["adjective","Jestem zadowolony ___. (praca)",["z pracy"],"доволен чем","Zadowolony z + Dopełniacz."],
 ["adjective","Jestem zmęczony ___. (praca)",["pracą"],"устал от чего","Zmęczony + Narzędnik без предлога.",1],
 ["adjective","Jestem pewien ___. (swoja racja)",["swojej racji"],"уверен в чём","Pewien и pewny требуют Dopełniacz: pewien swojej racji.",1],
 ["adjective","Jest podobny ___. (matka)",["do matki"],"похож на кого","Podobny do + Dopełniacz.",1],
 ["adjective","Jestem gotowy ___. (egzamin)",["na egzamin","do egzaminu"],"готов к чему","Gotowy na + Biernik о событии, gotowy do + Dopełniacz о действии.",1],
 ["adjective","Jestem zainteresowany ___. (oferta)",["ofertą"],"заинтересован в чём","Zainteresowany + Narzędnik без предлога.",1],
 ["adjective","Jestem dobry ___. (matematyka)",["w matematyce"],"силён в чём","Dobry w + Miejscownik. Так же słaby w czymś.",1],
 ["adjective","Sok bogaty ___. (witaminy)",["w witaminy"],"богат чем","Bogaty w + Biernik, а не Narzędnik.",1],
 ["adjective","Jestem odpowiedzialny ___. (projekt)",["za projekt"],"отвечает за что","Odpowiedzialny za + Biernik."],
 ["adjective","Jestem ci wdzięczny ___. (pomoc)",["za pomoc"],"благодарен за что","Wdzięczny + Celownik лица и za + Biernik повода."],
 ["adjective","Jest zdolny ___. (wszystko)",["do wszystkiego"],"способен на что","Zdolny do + Dopełniacz, а не «на».",1],
 ["adjective","Jestem chory ___. (angina)",["na anginę"],"болен чем","Chory na + Biernik.",1],
 ["adjective","Jestem zły ___. (siebie)",["na siebie"],"зол на кого","Zły na + Biernik."],
 ["adjective","Pokój pełen ___. (ludzie)",["ludzi"],"полон чего","Pełen и pełny требуют Dopełniacz."],
 ["adjective","Dochód wolny ___. (podatek)",["od podatku"],"свободен от чего","Wolny od + Dopełniacz."],
 ["noun","Mam problem ___. (komputer)",["z komputerem"],"проблема с чем","Problem z + Narzędnik."],
 ["noun","Mam ochotę ___. (kawa)",["na kawę"],"желание чего","Ochota na + Biernik: mam ochotę na kawę.",1],
 ["noun","Nie mam czasu ___. (to)",["na to"],"время на что","Czas na + Biernik."],
 ["noun","Nie ma powodu ___. (zmartwienia)",["do zmartwień"],"повод для чего","Powód do + Dopełniacz, а не «для».",1],
 ["noun","Masz prawo ___. (urlop)",["do urlopu"],"право на что","Prawo do + Dopełniacz, а не «на».",1],
 ["noun","To ma wpływ ___. (zdrowie)",["na zdrowie"],"влияние на что","Wpływ na + Biernik."],
 ["noun","To jest zgoda ___. (przetwarzanie danych)",["na przetwarzanie danych"],"согласие на что","Zgoda na + Biernik."],
 ["noun","Mam pomysł ___. (wakacje)",["na wakacje"],"идея чего","Pomysł na + Biernik: pomysł na wakacje.",1],
 ["noun","Mamy dostęp ___. (internet)",["do internetu"],"доступ к чему","Dostęp do + Dopełniacz.",1],
 ["noun","To strach ___. (latanie)",["przed lataniem"],"страх перед чем","Strach przed + Narzędnik."],
 ["noun","To tęsknota ___. (rodzina)",["za rodziną"],"тоска по чему","Tęsknota za + Narzędnik, как и глагол tęsknić.",1],
 ["noun","Ciągły brak ___. (czas)",["czasu"],"нехватка чего","Brak + Dopełniacz без предлога."],
 ["noun","To dobra okazja ___. (świętowanie)",["do świętowania"],"повод для чего","Okazja do + Dopełniacz.",1]
];
const NIEOKR = [
...
const PREPS = [
 ...
 ["obok / koło","Dopełniacz","рядом с, возле","obok sklepu, koło drzewa"],
...
const ALPHA_PRACTICE={id:"alphabet",title:"Практика: алфавит и произношение",lead:"20 заданий на буквы, сочетания, носовые, ассимиляцию, ударение и основные правила письма.",tasks:topicTasks("alpha",[
 ...
 ["Как читается ł в слове mały?","неслоговое у [w]","Ł обозначает губной [w], близкий к белорусскому ў; это не л.",["неслоговое у [w]","твёрдое л [ɫ]","мягкое ль [lʲ]"]],
 ["Какая буква произносится так же, как польское u?","ó","Ó и u обозначают один звук [u], но пишутся по разным орфографическим правилам.",["ó","y","o"]],
 ["Что представляет собой szcz в слове szczęście?","два диграфа и два звука: sz + cz","Szcz состоит из sz и cz и произносится [ʂt͡ʂ], это не один диграф и не русский щ.",["два диграфа и два звука: sz + cz","один диграф и один звук","одна буква с диакритикой"]],
 ["Какая запись передаёт звук dź [d͡ʑ] перед гласной в слове dziecko?","dzi","Перед гласной звук dź записывается триграфом dzi.",["dzi","dż","dz"]],
 ["Какова роль i в начале слова ciasto?","только обозначает мягкость c","В сочетании ci + другая гласная буква i отдельного слога не образует, а обозначает мягкость.",["только обозначает мягкость c","образует отдельный слог","делает c твёрдым"]],
 ["Какова роль i в начале слова cisza?","это гласная [i], которая также смягчает c","Если следующим звуком служит само i, оно произносится как гласная и одновременно показывает мягкость предыдущей согласной.",["это гласная [i], которая также смягчает c","это только знак мягкости без собственного звука","это согласный [j]"]],
 ["Как обычно реализуются ą / ę перед b и p?","[om] / [em]","Перед губными b и p появляется носовая согласная [m]: dąb [domp], zęby [zemby].",["[om] / [em]","[on] / [en]","[oŋ] / [eŋ]"]],
 ["Как обычно реализуются ą / ę перед t, d, c и dz?","[on] / [en]","Перед t, d, c и dz появляется [n]: kąt [kont], ręce [rence].",["[on] / [en]","[om] / [em]","чистые [o] / [e]"]],
 ["Как обычно реализуются ą / ę перед k и g?","[oŋ] / [eŋ]","Перед заднеязычными k и g появляется заднеязычный носовой [ŋ]: mąka [moŋka], ręka [reŋka].",["[oŋ] / [eŋ]","[on] / [en]","[om] / [em]"]],
 ["Что слышно в ą / ę перед щелевыми, например в wąs и węch?","носовой переход без отдельного n или m","Перед щелевыми сохраняется асинхронный носовой переход; вставлять отдельное n или m не нужно.",["носовой переход без отдельного n или m","обязательное n","обязательное m"]],
 ["Как произносится конечное ą в нейтральной норме, например в idą?","с носовым переходом, без отдельного n","Конечное ą сохраняет носовой компонент: учебно [idõũ̯], не [idon].",["с носовым переходом, без отдельного n","как сочетание on","как чистое o без носовости"]],
 ["Как обычно произносится конечное ę, например в proszę?","с ослабленной носовостью или как [ɛ]","У конечного ę носовость ослабляется и в обычной речи может исчезать; полная сильная носовость не является более тщательной нормой.",["с ослабленной носовостью или как [ɛ]","обязательно как en","обязательно с полной сильной носовостью"]],
 ["Что происходит с конечным b в произношении слова chleb?","оглушается до [p]","В конце слова звонкий препятственный обычно оглушается: chleb [chlep].",["оглушается до [p]","остаётся [b]","переходит в [v]"]],
 ["Почему prośba произносится приближённо [proźba]?","ś озвончается перед b","Перед звонким препятственным b глухое ś обычно озвончается до ź.",["ś озвончается перед b","b оглушается до p","ś исчезает"]],
 ["Как читается предлог w в сочетании w parku?","как [f]","Ассимиляция переходит границу слов: перед глухим p предлог w оглушается до [f].",["как [f]","как [v]","как [w]"]],
 ["На какой слог падает ударение в слове Warszawa?","на sza: War-SZA-wa","По основному правилу ударение падает на предпоследний слог.",["на sza: War-SZA-wa","на War: WAR-sza-wa","на wa: War-sza-WA"]],
 ["Как правильно распределено ударение в двух формах?","BY-li-śmy; zro-BI-li-by-śmy","В прошедших формах на -śmy ударение нормативно падает на третий слог от конца, в условных 1-го лица множественного - на четвёртый.",["BY-li-śmy; zro-BI-li-by-śmy","by-LI-śmy; zro-bi-LI-by-śmy","by-li-ŚMY; zro-bi-li-by-ŚMY"]],
...
const ASPEKT_UWAGI = {
 ...
 "siedzieć": "смена положения: siadać / usiąść",
...
const GROUPS = [
  ...
  ["Игры", [
    ["s-games","Все игры и как они связаны со справочником"],
    ["s-mil","Пятнадцать вопросов, три подсказки, две несгораемые суммы"],
    ["s-sek","60 секунд на род, вид глагола и орфографию"],
    ["s-det","Найдите ошибку в предложении или докажите, что её нет"],
    ["s-zd","Соберите фразу из слов, среди которых есть лишние"],
  ]],
  ["О проекте", [
...
const LADDER_SAFE = [5, 10];

const SPEED_CLOCK = {start:60, bonus:2, penalty:5};
const SPEED_SKIP = [
 "aby","ażeby","byleby","chociażby","choćby","czyżby","gdyby","jakby","jakoby","niby","żeby","oby",
 "polski","Burek","Stare Miasto","Nowy Rok","Sejm"
];
const SPEED_ORTHO = [
 ["st(ó)ł","stół → stoły: ó чередуется с o"],["s(ó)l","sól → soli: ó чередуется с o"],["r(ó)g","róg → rogi: ó чередуется с o"],
 ["w(ó)z","wóz → wozy: ó чередуется с o"],["d(ó)ł","dół → doły: ó чередуется с o"],["ogr(ó)d","ogród → ogrody: ó чередуется с o"],
 ["samoch(ó)d","samochód → samochody: ó чередуется с o"],["m(ó)j","mój → moja: ó чередуется с o"],["tw(ó)j","twój → twoja: ó чередуется с o"],
 ["sw(ó)j","swój → swoja: ó чередуется с o"],["dw(ó)r","dwór → dworu: ó чередуется с o"],["wiecz(ó)r","wieczór → wieczory: ó чередуется с o"],
 ["p(ó)ł","pół → połowa: ó чередуется с o"],["pok(ó)j","pokój → pokoje: ó чередуется с o"],["mi(ó)d","miód → miodu: ó чередуется с o"],
 ["wr(ó)cić","wracać → wrócić: ó чередуется с a"],["ni(ó)sł","niesie → niósł: ó чередуется с e"],
 ["student(ó)w","окончание -ów"],["dom(ó)w","окончание -ów"],["Krak(ó)w","окончание -ów"],
 ["poczt(ó)wka","суффикс -ówka"],["stoł(ó)wka","суффикс -ówka"],["(ó)smy","ó в начале слова: одна из пяти основ"],["(ó)semka","ó в начале слова: одна из пяти основ"],
 ["g(ó)ra","без чередования: написание запоминается"],["kr(ó)l","без чередования: написание запоминается"],["m(ó)zg","без чередования: написание запоминается"],
 ["pr(ó)ba","без чередования: написание запоминается"],["wr(ó)bel","без чередования: написание запоминается"],["wsp(ó)lny","без чередования: написание запоминается"],
 ["dom(u)","в конце слова всегда u"],["ojc(u)","в конце слова всегда u"],["t(u)","в конце слова всегда u"],["tem(u)","в конце слова всегда u"],
 ["(u)lica","в начале слова u, кроме пяти основ с ó"],["(u)czeń","в начале слова u, кроме пяти основ с ó"],["(u)waga","в начале слова u, кроме пяти основ с ó"],
 ["mal(u)tki","суффикс -utki"],["opiek(u)n","суффикс -un"],["podar(u)nek","суффикс -unek"],["leni(u)ch","суффикс -uch"],["ham(u)lec","суффикс -ulec"],["mal(u)szek","суффикс -uszek"],
 ["prac(u)ję","формы глаголов на -ować: pracuję, pracujesz"],["dzięk(u)ję","формы глаголов на -ować: dziękuję, dziękujesz"],["kup(u)jesz","формы глаголов на -ować: kupuję, kupujesz"],
 ["sk(u)wka","производное от kuć: u не меняется"],["zas(u)wka","производное от suwać: u не меняется"],
 ["p(rz)yjaciel","после p пишется rz"],["t(rz)y","после t пишется rz"],["k(rz)esło","после k пишется rz"],["g(rz)yb","после g пишется rz"],
 ["w(rz)esień","после w пишется rz"],["b(rz)eg","после b пишется rz"],["d(rz)wi","после d пишется rz"],
 ["mala(rz)","суффикс -arz"],["leka(rz)","суффикс -arz"],["kucha(rz)","суффикс -arz"],["tance(rz)","суффикс -erz"],["żołnie(rz)","суффикс -ierz"],
 ["dwo(rz)ec","dwór → dworzec: rz чередуется с r"],["na gó(rz)e","góra → na górze: rz чередуется с r"],["mo(rz)ski","morze → morski: rz чередуется с r"],
 ["(rz)eka","без правила: написание запоминается"],["(rz)ecz","без правила: написание запоминается"],
 ["ksią(ż)ka","księga → książka: ż чередуется с g"],["odwa(ż)ny","odwaga → odważny: ż чередуется с g"],["wa(ż)ny","waga → ważny: ż чередуется с g"],
 ["dro(ż)szy","drogi → droższy: ż чередуется с g"],["dru(ż)yna","druh → drużyna: ż чередуется с h"],["gro(ż)ę","groźny → grożę: ż чередуется с ź"],
 ["mro(ż)ony","mróz → mrożony: ż чередуется с z"],["mał(ż)eństwo","после ł пишется ż"],["skar(ż)yć","после r пишется ż"],["oran(ż)ada","после n пишется ż"],
 ["s(ch)ody","после s пишется ch"],["s(ch)ab","после s пишется ch"],["s(ch)owek","после s пишется ch"],
 ["w miasta(ch)","окончание Miejscownik множественного -ach"],["o stoła(ch)","окончание Miejscownik множественного -ach"],["na ulica(ch)","окончание Miejscownik множественного -ach"],
 ["da(ch)","на конце исконных слов ch"],["grze(ch)","на конце исконных слов ch"],["mu(ch)a","mucha → muszka: ch чередуется с sz"],
 ["su(ch)y","suchy → susza: ch чередуется с sz"],["pocie(ch)a","pociecha → pocieszyć: ch чередуется с sz"],["u(ch)o","ucho → uszy: ch чередуется с sz"],
 ["(ch)emia","заимствование: написание словарное"],["(ch)arakter","заимствование: написание словарное"],["me(ch)anik","заимствование: написание словарное"],
 ["(ch)leb","без правила: написание запоминается"],["(ch)cieć","без правила: написание запоминается"],
 ["wa(h)a się","waga → waha się: h чередуется с g"],["dru(h)","druh → drużyna: h чередуется с ż; исключение из ch на конце слова"],
 ["(h)igiena","заимствование: написание словарное"],["(h)otel","заимствование: написание словарное"],["(h)arfa","заимствование: написание словарное"],
 ["(h)istoria","заимствование: написание словарное"],["(h)erbata","заимствование: написание словарное"],["(h)umor","заимствование: написание словарное"],["(h)ymn","заимствование: написание словарное"]
];
...
},
{
  ...
  tasks:[
    ...
    {id:"adjuse-8", prompt:"Мама и папа справятся - Razem jesteście ___.", options:["silni","silnymi","silne"], answers:["silni"], explanation:"После być прилагательное без существительного остаётся в именительном. В группе есть мужчина, поэтому мужско-личная форма: silni."},
    {id:"adjuse-9", prompt:"Dziecko płacze, bo jest ___.", options:["głodne","głodnym","głodny"], answers:["głodne"], explanation:"Dziecko - средний род, после być прилагательное стоит в именительном: głodne."},
    {id:"adjuse-10", prompt:"Dziewczyny wróciły z gór i były bardzo ___. (zmęczony)", answers:["zmęczone"], explanation:"Немужско-личное множественное в именительном: zmęczone, не zmęczonymi."},
    {id:"adjuse-11", prompt:"Ewa jest ___. (zdolna studentka)", answers:["zdolną studentką"], explanation:"Появилось существительное - вся группа уходит в творительный: zdolną studentką. Без него было бы Ewa jest zdolna."},
...
const WSZ_PHR = [
 ...
 ["Wszystkie bilety są sprzedane.","Все билеты проданы.","не-мужско-личное - о вещах"],
